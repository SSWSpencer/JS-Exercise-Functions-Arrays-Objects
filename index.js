 ...
 * }
*/
function makePersonObject(identity, nm, em) {
  const personObj = {
    id: identity,
    name: nm,
    email: em
...
function makeSmartPerson(nm) {
  let smartPers = {
    ...
    speak: function(){
      return "Hello, my name is " + smartPers.name;
    }
  };
  return smartPers;
}
...
function getCarInfoById(carInv, carId) {
  return "This is a " + carInv[carId-1].car_make + " " + carInv[carId-1].car_model;
}
...
*/
function sortCarInventory(carInv) {
...
*/
function getModelYears(carInv) {
  let yearArr = [];
  for(let i = 0; i < carInv.length; i++){
    yearArr.push(carInv[i].car_year);
  }
  return yearArr;
}
...
*/
function getOlderCars(carInv, maxYear) {
  let oldCars = [];
  for(let i = 0; i < carInv.length; i++){
    if(carInv[i].car_year <= maxYear){
      oldCars.push(carInv[i]);
    }
  }
  return oldCars;
}
 ...
 * in the same order as they appear in the original inventory.
*/
function getGermanCars(carInv) {
  let germanCars = [];
  for(let i = 0; i < carInv.length; i++){
    if(carInv[i].car_make == "Volkswagen" || carInv[i].car_make == "Audi" || carInv[i].car_make == "Mercedes-Benz" || carInv[i].car_make == "BMW"){
      germanCars.push(carInv[i]);
    }
  }
  return germanCars;
}
...
function carMaker(num) {
  let theCar = {
    odometer: num,
    drive: function(dist){
      theCar.odometer+= dist;
      return theCar.odometer;
    }
  }
  return theCar;
}
